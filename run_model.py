...
def simpson(img_name, cred):
    ...
    url = blob.generate_signed_url(datetime.timedelta(
        seconds=300), method='GET')  # blob에서 url을 빼온뒤
    image_nparray = np.asarray(bytearray(requests.get(
    ...
    c_image = cv2.resize(c_image, (64, 64))  # 이미지를 데이터에 맞게 리사이즈 해줍니다
